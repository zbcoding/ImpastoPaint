...
| Triangle shape tool, added as a 5th entry to the existing shape-type list (Open/Closed Line-Curve Series, Ellipse, Rounded Line Series). Holding Ctrl while drawing makes an equilateral triangle (apex up, level base centered underneath) instead of a right triangle | `TriangleTool.cs`, `TriangleEditEngine.cs` (new), `BaseEditEngine.cs`, `ShapeEngineCollection.cs` | build + unit tests, app starts; **not interactively hand-verified** |
...
| Responsive status bar / colors bar folding as the window narrows: cursor-position and image-size labels dim then hide, then quick/recent-colors swatches and finally primary/secondary swatches fold out of the docked palette bar into the color-wheel popover | `ActionManager.cs`, `StatusBarColorPaletteWidget.cs`, `MainWindow.cs` | build + unit tests; **narrow-window folding not hand-verified on screen** |
